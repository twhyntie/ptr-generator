...
def format_datetime(year, month, day, hour, minute, second):
	# Format date / time
    datetime = str(year).zfill(2)+"-"+str(month).zfill(2)+"-"+str(day).zfill(2)+"T"+str(hour).zfill(2)+":"+str(minute).zfill(2)+":"+str(second).zfill(2)
    return datetime
	
def format_duration(day, hour, minute, second):
	dur = day.zfill(2)+"T"+hour.zfill(2)+":"+minute.zfill(2)+":"+second.zfill(2)
	return dur
...
if ((len(sys.argv) - 1) != 25):
	...
	explan_text = ""
	explan_text = sys.argv[25].strip() # Input 25
	
	#Generate issue date from current date and time
	ctime = datetime.datetime.now()
	issuedate = format_datetime(ctime.year, ctime.month, ctime.day, ctime.hour, ctime.minute, ctime.second)
	...
	<ptrFile xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:noNamespaceSchemaLocation="PtrFile.xsd">
		<fileHeader>
			...
			<issueDate>"""+str(issuedate)+"""</issueDate>		
